...
export function useSocialViews(deps: EigoMasterViewDeps) {
  ...
  const RankingScreen = () => {
    ...
    return <div className="sa" style={{ padding: 16 }}>
      ...
      {!rankingLoading && rankingData.length === 1 && rankingData[0]?.isSelfFallback && <div className="jp" style={{ fontSize: 12, color: 'var(--t3)', marginBottom: 8 }}>この期間・種別ではランキング対象の実績データがまだありません。自分の行だけ仮表示しています。</div>}
  ...
  const Settings = () => {
    ...
    return <div className="sa">
      <div className="stlist">
        ...
        <div className="stst">おすすめ学習サービス</div>
        <StudySapuriCard screenName="settings" placement="settings" variant="home" compact />
        <StudySapuriCard screenName="settings" placement="settings" service="cambly" variant="home" compact />
