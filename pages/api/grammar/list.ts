...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const attempts = userId && dbIds.length
    ? await sbGet(`grammar_attempts?select=*&user_id=eq.${encodeURIComponent(userId)}&question_id=in.(${dbIds.join(',')})&order=created_at.desc&limit=1000`)
    : [];
  const latest = latestByQuestion(attempts);

  const explanations = dbIds.length
    ? await sbGet(`grammar_explanations?select=*&question_id=in.(${dbIds.join(',')})&order=score.desc,created_at.asc&limit=1000`)
    : [];
  const explanationIds = explanations.map((e: any) => e.id).filter(Boolean);
  const explanationVotes = explanationIds.length
    ? await sbGet(`grammar_explanation_votes?select=*&explanation_id=in.(${explanationIds.join(',')})&limit=5000`)
    : [];
  const authorIds = [...new Set(explanations.map((e: any) => e.author_user_id).filter(Boolean))];
  const profiles = authorIds.length
    ? await sbGet(`user_profiles?select=user_id,nickname,avatar_emoji&user_id=in.(${authorIds.map(encodeURIComponent).join(',')})&limit=1000`)
    : [];
