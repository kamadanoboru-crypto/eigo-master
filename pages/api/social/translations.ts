...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    ...
    const r = await fetch(`${SB_URL}/rest/v1/user_translations?select=*&${filter}`, { headers: headers() });
  ...
  if (req.method === 'POST') {
    ...
    const r = await fetch(`${SB_URL}/rest/v1/user_translations`, {
      method: 'POST',
    ...
    if (!r.ok) {
      ...
      console.error('[social] translation insert failed', r.status, detail.slice(0, 300));
      return res.status(500).json({ error: 'translation save failed' });
    }
