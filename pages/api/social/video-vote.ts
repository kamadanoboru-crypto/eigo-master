...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!SB_URL || !SB_ANON) return res.status(200).json({ ok: true, vote: voteType, likes: 0, dislikes: 0 });
  ...
  const likes = rows.filter((row: any) => Number(row.vote_type) === 1).length;
  const dislikes = rows.filter((row: any) => Number(row.vote_type) === -1).length;
